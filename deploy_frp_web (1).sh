...
def config_frps():
    if request.method == 'POST':
        ...
        enable_dashboard = request.form.get('enable_dashboard') == 'on'
        dashboard_port = request.form.get('dashboard_port', '7500') if enable_dashboard else ''
        dashboard_user = request.form.get('dashboard_user', 'admin') if enable_dashboard else ''
        dashboard_pwd = request.form.get('dashboard_pwd', 'admin') if enable_dashboard else ''
        ...
        if enable_dashboard:
            config['common']['dashboard_port'] = int(dashboard_port)
            config['common']['dashboard_user'] = dashboard_user
            config['common']['dashboard_pwd'] = dashboard_pwd
...
def config_frpc():
    if request.method == 'POST':
        ...
        for i in range(client_count):
            ...
            if not local_port or not service_name:
                continue
            ...
            elif proxy_type == 'tcp' and remote_port:
    ...
    if os.path.exists(FRPC_CONFIG):
        try:
            with open(FRPC_CONFIG, 'r') as f:
                config = tomlkit.load(f)
            clients = []
            for name, proxy in config.get('proxy', {}).items():
                clients.append({
                    'service_name': name,
                    'local_ip': proxy.get('local_ip', '127.0.0.1'),
                    'local_port': str(proxy.get('local_port', '')),
                    'proxy_type': proxy.get('type', 'tcp'),
                    'custom_domains': proxy.get('custom_domains', ''),
                    'subdomain': proxy.get('subdomain', ''),
                    'remote_port': str(proxy.get('remote_port', ''))
                })
        except Exception as e:
            flash(f'读取配置文件失败: {str(e)}', 'danger')
    return render_template('config_frpc.html', config=config, clients=clients)
...
<body>
    <div class="container mt-5">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="enable_dashboard" name="enable_dashboard" {{ 'checked' if config.get('common', {}).get('dashboard_port') else '' }}>
                <label class="form-check-label" for="enable_dashboard">启用 Dashboard</label>
            </div>
            <div class="mb-3">
                <label for="dashboard_port" class="form-label">Dashboard 端口（例如 7500，需确保未被占用）</label>
                <input type="text" class="form-control" id="dashboard_port" name="dashboard_port" value="{{ config.get('common', {}).get('dashboard_port', '7500') }}">
            </div>
            <div class="mb-3">
                <label for="dashboard_user" class="form-label">Dashboard 用户名</label>
                <input type="text" class="form-control" id="dashboard_user" name="dashboard_user" value="{{ config.get('common', {}).get('dashboard_user', 'admin') }}">
            </div>
            <div class="mb-3">
                <label for="dashboard_pwd" class="form-label">Dashboard 密码</label>
                <input type="text" class="form-control" id="dashboard_pwd" name="dashboard_pwd" value="{{ config.get('common', {}).get('dashboard_pwd', 'admin') }}">
            </div>
        ...
        <form method="POST">
            ...
            <div id="clients_container">
                ...
                <div class="client-section mb-4 p-3 border rounded" id="client_{{ loop.index0 }}">
                    ...
                    <div id="http_fields_{{ loop.index0 }}" style="display: {{ 'block' if client.proxy_type in ['http', 'https'] else 'none' }};">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="subdomain_{{ loop.index0 }}" class="form-label">二级域名前缀（仅当服务端设置了二级域名主机时有效，例如 test）</label>
                            <input type="text" class="form-control" id="subdomain_{{ loop.index0 }}" name="subdomain_{{ loop.index0 }}" value="{{ client.subdomain }}">
                        </div>
                    </div>
                    <div id="tcp_fields_{{ loop.index0 }}" style="display: {{ 'block' if client.proxy_type == 'tcp' else 'none' }};">
                        <div class="mb-3">
        ...
        function addClient() {
            ...
            clientDiv.innerHTML = \`
                ...
                <div id="http_fields_\${clientCount}" style="display: none;">
                    ...
                    <div class="mb-3">
                        <label for="subdomain_\${clientCount}" class="form-label">二级域名前缀（仅当服务端设置了二级域名主机时有效，例如 test）</label>
...
# 检查 Dashboard 默认端口（7500）
if check_port "$DASHBOARD_PORT_DEFAULT" && [[ "$INSTALL_MODE" == "both" || "$INSTALL_MODE" == "frps" ]]; then
    echo -e "${RED}警告：Dashboard 默认端口 $DASHBOARD_PORT_DEFAULT 已被占用！${NC}"
    read -p "是否清理 $DASHBOARD_PORT_DEFAULT 端口的占用？（y/n，默认 n）： " CLEAR_DASHBOARD_PORT
    CLEAR_DASHBOARD_PORT=${CLEAR_DASHBOARD_PORT:-n}
    if [[ "$CLEAR_DASHBOARD_PORT" =~ ^[Yy]$ ]]; then
        clear_port "$DASHBOARD_PORT_DEFAULT"
    else
        echo "跳过清理 $DASHBOARD_PORT_DEFAULT 端口，Web 界面将提示用户选择其他端口。"
    fi
fi
...
if command -v ufw >/dev/null 2>&1; then
    echo "配置 ufw 防火墙以允许 Web 界面和 Dashboard 端口..."
    ufw allow "$PYTHON_PORT"/tcp
    if [[ "$INSTALL_MODE" == "both" || "$INSTALL_MODE" == "frps" ]]; then
        ufw allow 7000/tcp  # 默认 frps 绑定端口
        ufw allow 7500/tcp  # 默认 Dashboard 端口
    fi
    ufw status
elif command -v firewall-cmd >/dev/null 2>&1; then
    echo "配置 firewalld 防火墙以允许 Web 界面和 Dashboard 端口..."
...
if [[ "$INSTALL_MODE" == "both" || "$INSTALL_MODE" == "frps" ]]; then
    echo "Dashboard 地址（启用后）: http://$SERVER_LAN_IP:7500/ 或 http://$SERVER_WAN_IP:7500/"
fi
echo "Web 界面认证信息："
echo "用户名: $USERNAME"
echo "密码: $PASSWORD"
...
echo -e "${GREEN}通过 Web 界面可配置服务端（frps）或客户端（frpc），支持 TCP 转发、HTTPS（需上传 SSL 证书）和 Dashboard！${NC}"